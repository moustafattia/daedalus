...
class AgenticConfig:
    ...
    def from_raw(cls, *, raw: dict[str, Any], workflow_root: Path) -> "AgenticConfig":
        ...
        state_path = _resolve(root, str(storage_raw.get("state", ".daedalus/agentic-state.json")))
        audit_log_path = _resolve(root, str(storage_raw.get("audit-log", ".daedalus/agentic-audit.jsonl")))
